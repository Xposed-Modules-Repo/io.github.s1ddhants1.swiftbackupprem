...
Swift Backup Premium LSPosed module (tested on v4.2.3, v4.2.5 and v5.0.4, v5.1.0 but should also work on newer versions - thanks to [DexKit](https://github.com/LuckyPray/DexKit)).
...
Setting up Cloud Storage is Optional / Skip if paid
As Firebase Cloud Storage now requires a paid **Blaze Plan** (linked Cloud Billing account). **You can safely skip enabling Cloud Storage in Firebase Console.**
Swift Backup does **not** store your backup files (APKs, app data, call logs) inside Firebase Storage. Firebase is only used for authentication and metadata synchronization.
