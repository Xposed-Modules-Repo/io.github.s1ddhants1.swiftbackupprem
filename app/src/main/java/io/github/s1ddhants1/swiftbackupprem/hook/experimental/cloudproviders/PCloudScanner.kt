...
object PCloudScanner : CloudScanner {
    ...
    override fun listFiles(context: Context, prefs: SharedPreferences): List<CloudFileItem> {
        ...
        val respText = executeGet(urlStr, token) ?: return emptyList()
    ...
    override fun downloadFileText(context: Context, prefs: SharedPreferences, fileItem: CloudFileItem): String? {
        ...
        val respText = executeGet(linkUrl, token) ?: return null
    ...
    private fun executeGet(urlStr: String, token: String? = null): String? = attempt("pCloud HTTP GET", silent = true) {
        ...
        try {
            if (conn.responseCode in 200..299) {
                ...
                Log.w(TAG, "[PCloudScanner] HTTP GET returned ${conn.responseCode} for ${AppUtils.sanitizeUrl(urlStr)}")
                null
            }
        } finally {
            conn.disconnect()
